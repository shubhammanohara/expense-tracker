...
export default function App() {
  const [activeTab, setActiveTab] = useState<'home' | 'add' | 'history' | 'settings'>('home');
  ...
  const renderScreen = () => {
    switch (activeTab) {
      ...
      case 'add': return <AddExpense onCancel={() => setActiveTab('home')} onSuccess={() => setActiveTab('home')} />;
...
function NavButton({ active, onClick, icon, label }: NavButtonProps) {
  return (
    ...
    >
      <div className={active ? '[&>svg]:fill-primary' : ''}>
        {icon}
      </div>
      <span className="font-body text-[10px] font-semibold uppercase tracking-widest mt-1">{label}</span>
    </button>
  );
}
